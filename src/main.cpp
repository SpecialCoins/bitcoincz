...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    bool fColdStakingActive = !sporkManager.IsSporkActive(SPORK_26_COLDSTAKING_MAINTENANCE);
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            fColdStakingActive = !sporkManager.IsSporkActive(SPORK_26_COLDSTAKING_MAINTENANCE);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_21_MASTERNODE_PAYMENT_ENFORCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_22_MASTERNODE_PAYMENT) ||
                              !pSporkDB->SporkExists(SPORK_32_F_PAYMENT_NEW) ||
                              !pSporkDB->SporkExists(SPORK_24_F_PAYMENT_ENFORCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_26_COLDSTAKING_MAINTENANCE);
...
int ActiveProtocol()
{
    int ActiveProtocol = PROTOCOL_VERSION;
