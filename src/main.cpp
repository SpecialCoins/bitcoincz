                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (sporkManager.IsSporkActive(SPORK_20_ZERO_FEES) && nFees == 0) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    const int64_t nMinted = (nValueOut - nValueIn) + nFees;
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight) + nFees + (0.00000001 * COIN);;
    if (block.IsProofOfWork())
        nExpectedMint += nFees;

    //Check that the block does not overmint
    if (!IsBlockValueValid(pindex->nHeight, nExpectedMint, nMinted) && pindex->pprev->nHeight + 1 > UTXOF) {
        return state.DoS(100, error("ConnectBlock() : reward pays too much (actual=%s vs limit=%s)",
                                    FormatMoney(nMinted), FormatMoney(nExpectedMint)),
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if (!consensus.NetworkUpgradeActive(pindexNew->nHeight, Consensus::UPGRADE_MODV2)) {
...
{
    if (sporkManager.IsSporkActive(SPORK_31_SAFETY_KILL_NEW))
    {
