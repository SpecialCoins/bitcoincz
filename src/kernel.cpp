    ...
    stakeValue(stakeInput->GetValue())
{
    ...
    if (!Params().GetConsensus().NetworkUpgradeActive(pindexPrev->nHeight + 1, Consensus::UPGRADE_MODV2)) {
...
bool Stake(const CBlockIndex* pindexPrev, CStakeInput* stakeInput, unsigned int nBits, int64_t& nTimeTx)
{
    ...
    if (!consensus.HasStakeMinAgeOrDepth(prevHeight + 1, nHeightBlockFrom))
        return error("%s : min age violation - height=%d, nHeightBlockFrom=%d",
                         __func__, prevHeight + 1, nHeightBlockFrom);
...
bool CheckProofOfStake(const CBlock& block, std::string& strError, const CBlockIndex* pindexPrev)
{
    ...
    const CAmount& nValueIn = stakeInput->GetValue();
    if ((nValueIn < 100) && (masternodeSync.IsBlockchainSynced()))
    {
        return false;
    }
