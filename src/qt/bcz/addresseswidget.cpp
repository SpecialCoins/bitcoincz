    ...
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter BCZ address"));
    setCssProperty(ui->labelAddress, "text-title");
    ui->lineEditAddress->setPlaceholderText("e.g. B52EagiPxecjS9zwyebbCZz3x3QuYBNezo");
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress bczAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(bczAdd.Get(), label.toUtf8().constData(),
                bczAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
