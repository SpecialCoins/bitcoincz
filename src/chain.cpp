...
void CBlockIndex::SetNewStakeModifier(const uint256& prevoutId)
{
    ...
    if (!Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_MODV2)) return;
...
{
    if (vStakeModifier.empty() || Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_MODV2))
        return 0;
